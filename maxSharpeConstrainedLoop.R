...
for (i in seq(1,numberOfLoops)) {
	...
	args = c(paste(dataPaths[1], ".csv", sep=''),
		paste(dataPaths[2:4], "_", toString(i), ".csv", sep='') # .../directory/data_iterationNumber.csv
		)
	...
	if (length(args)==0) {
			...
			writeLines(paste('\n\tINITIALIZING ITERATION ',toString(i),'.\n',sep=''))
	...
	writeLines(paste('\tBEGINNING COMPUTATION ON ITERATION ',toString(i),'...\n',sep=''))
	...
	if (i == 1) { # initialize an empty df
		...
		masterData = data.frame(
			fields=c('minAllocation','maxAllocation','expectedReturn','volatility','sharpeRatio',assetNames),
			row.names='fields'
			)
...
file='./sharpeLoops.csv', # write it to current directory
row.names=TRUE) # include the row names

writeLines('\tSUCCESS. RESULTS WRITTEN TO CURRENT DIRECTORY AS sharpeLoops.csv!\n')
