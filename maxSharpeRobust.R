...
if (length(args)==0) {
		...
		stop('ERROR: NO DATA FILEPATH PROVIDED.n', call=TRUE)
		...
		stop('ERROR: CAN ONLY SUPPLY ONE FILEPATH ARGUMENT.n', call=TRUE)
...
covar = cov(returns, use="complete.obs", method="spearman") # robust covariance; casewise deletion of missing data
...
returns = apply(returns, 2, trimMean) # overwrite master dataframe to save space

remove(eig,minEig,maxEig,condNum,reg) # garbage collection

### QUADRATIC PROGRAMMING

A = rbind(returns, rep(1,n), diag(n) - 2e-4, 66e-4 - diag(n)) # left constraints equation matrix
b0 = matrix(c(1,rep(0,2*n+1)),ncol=1) # right (in)equality vector
...
weights = round(500*qp/sum(qp),7) # portfolio sum 500
...
write.csv(exportData, # export the weights as csv
	file='./maxSharpeRobust.csv', # write it to current directory
...
write.csv(data.frame(meanRet, volatility, sharpeRatio), # dataframe with portfolio stats
	file='./portfolioStatsRobust.csv', # write it to current directory
	row.names=FALSE) # do not include the row index

writeLines('\tSUCCESS. RESULTS WRITTEN TO CURRENT DIRECTORY AS maxSharpeRobust.csv AND portfolioStatsRobust.csv!\n')
